...
def _nightly_step(d, prices, factors, rets, broker, ledger, reconciler, prev_book, window,
                  notional: float = NOTIONAL) -> dict:
    """One trading day through the paper machinery. Shared by --dry-run and --live so the live path
    reuses tested logic; returns today's book (to carry as tomorrow's prev_book). `notional` is the
    book's gross exposure (sum|w|=1), sized to the account by the live caller."""
...
def _fetch_alpaca_panel(window: int, lookback_days: int = 90):
    ...
    # Alpaca uses '.' for class shares (BRK.B); the Wikipedia composite uses '-' (BRK-B)
    ...
    etfs = [e for e in etfs if e in tradable]
    print(f"universe: {len(universe)}/{len(sectors)} S&P 500 names tradable on Alpaca")

    start = pd.Timestamp.today().normalize() - pd.tseries.offsets.BDay(window + lookback_days)
    bars = dc.get_stock_bars(StockBarsRequest(symbol_or_symbols=universe + etfs,
                                              timeframe=TimeFrame.Day, start=start))
    close = bars.df["close"].unstack("symbol")
    close.index = pd.to_datetime(close.index).tz_localize(None).normalize()
    prices = close[[c for c in universe if c in close.columns]]
...
def live_run(window: int, out_root: Path) -> dict:
    ...
    if errs:
        print(f"{len(errs)} order(s) rejected (e.g. {errs[0]['ticker']}: {errs[0]['error'][:80]})")
    rep = bracket_report(ledger.read("daily_nav"), ledger.read("positions"))
    (out_root / "scorecard.md").write_text(to_markdown(rep))
    return rep
...
def flatten() -> None:
    """Reset the paper book: cancel open orders + close all positions. Market orders, so run during
    market hours for immediate effect; otherwise they queue for the next open."""
    _load_dotenv()
    if not (os.environ.get("ALPACA_API_KEY_ID") and os.environ.get("ALPACA_API_SECRET_KEY")):
        sys.exit("--flatten needs ALPACA keys in the env (or alpha-lab/.env)")
    from core.broker.alpaca import alpaca_paper_broker
    tc = alpaca_paper_broker(price_fn=lambda t: None)._tc
    tc.cancel_orders()
    tc.close_all_positions(cancel_orders=True)
    print("flattened: cancelled open orders + submitted closes for all positions")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--check", action="store_true", help="verify Alpaca paper keys authenticate, no trading")
    ap.add_argument("--flatten", action="store_true", help="reset paper book: cancel orders + close positions")
    ...
    if args.flatten:
        flatten()
        return
    # dry-run (throwaway synthetic replay) and live (the persistent book) must NOT share ledgers
    if args.live:
        out_root = Path("artifacts/statarb/paper/live")
        rep = live_run(args.window, out_root)
    elif args.dry_run:
        out_root = Path("artifacts/statarb/paper/dryrun")
