# hunt2026 trial ledger — every hypothesis, every loop, every adaptive step

The number that deflates a result is not "how many specs ran" but "how many chances the
process gave itself." This ledger counts both. Benchmarks are controls, not trials.

## Hunt-level table

| Hunt | Date | Hypotheses tested | Derived from prior holdout results? | Finalists | Pass bar | Bar |
|---|---|---|---|---|---|---|
| Hunt 1 (1y blind) | 2026-07-10 | 14 | no (research from train ≤2025-07 only) | 14 | 11 raw / 4 by excess | ≥+18% net, 1y |
| Hunt 2 (5y blind) | 2026-07-10 | 4 | **yes, designs chosen after seeing Hunt-1 5y stress** | 4 | 3 | ≥+18% CAGR, 5y |

**Effective trial count: > 18.** Hunt 2 was an adaptive loop: its 4 designs were selected
knowing which Hunt-1 families died in the 2022 window. Formally the process had 18 registered
chances plus one selection step over the design space; every future hunt that reacts to a
holdout result must add a row here and say so in this column.

## Robustness experiments (registered probes of frozen specs — not new spec trials)

| Exp | Date | Variants registered | Derived from prior holdout results? | Decisive stat |
|---|---|---|---|---|
| defensive-asset (dual_momentum_gold third-asset menu) | 2026-07-10 | 10 (GLD/TLT/DBC/XLU/XLP/UUP/SLV/VNQ/NONE/EQW-def) | **yes, adaptive loop: reacts to hunt-2 blind result + ledger's "gold hindsight" flag** | GLD-vs-NONE window win share, split pre/post 2024-01-01 (preregistrations/defensive-asset-2026-07-10.md), **result: REGIME ARTIFACT** (GLD wins 13% of pre-2024 windows; robustness/defensive_asset.md); live book unchanged, Stage-4 flag |
| ic-earnings-fwd (layer A, new info source: point-in-time earnings surprises) | 2026-07-10 | 1 primary (SUE→20d rank IC) + 1 secondary (day+1 confirmation) + 2 pre-registered conditioners (sector-relative, dispersion regime) | no, literature prior (PEAD); forward-only collection, no hunt2026 holdout input | 20d rank IC ≥ 0.03 & t ≥ 2 at n ≥ 300 pt-in-time events; kill IC < 0.01 or t < 1 at n ≥ 600 (preregistrations/exp-ic-earnings-fwd-2026-07-10.md), **ACCUMULATING** from 2026-07-10 |
| est-crossover (layer B, JSE window-length crossover + ψ̂ predictor, Estimator Lab) | 2026-07-10 | 6 window lengths (n=42/63/90/126/189/252; k=3; jse3 vs pca3 matched pair, both books) | **yes, adaptive loop: reacts to F-021 n=252 fail + its n=63 reopen; endpoints already seen, the 4 interior windows + ψ̂ predictor test are new** | long-only paired t per n + pooled Spearman(monthly Δ, median ψ̂), pre-committed ψ̂ cuts 0.90/0.95 (preregistrations/est-crossover-2026-07-10.md), **result: NO CROSSOVER** (long-only JSE helps at all n, −2.6→−0.5 bps monotone; ψ̂ has zero within-n timing content, both cuts rejected; F-021 CLOSED, estimator_lab/CROSSOVER.md) |
| ops-reality (layer D, execution vs frozen cost model) | 2026-07-10 | 0 spec variants (read-only measurement harness; thresholds fixed before any fill existed) | no, operational, reacts to go-live not to any performance number | trailing-mean slippage in [0,15] bps stocks / [0,5] bps ETFs; reject rate < 2%/night; per-book tracking drag < 30 bps/month; silent-flat alarms = 0 (preregistrations/ops-reality-2026-07-10.md), **ACCUMULATING** nightly in ledgers/hunt2026/_reconcile.jsonl |
| turnover-band (layer C, portfolio no-trade band overlay on the vol-managed family) | 2026-07-14 | 12 (band ∈ {0.01,0.02,0.05,0.10,0.20,0.40} L1 × {vol_managed_qqq, vol_core_svxy}) + 2 band=0 baselines | **yes, adaptive loop: targets blind-promoted books; effect sized from their published cost-drag/turnover** | per (book, band): median 12m-window net delta (banded − baseline) ≥ +5 bps AND turnover cut ≥ 20% = helps; all within ±5 bps = flat; all ≤ 0 = harmful → FAILURES.md (preregistrations/turnover-band-2026-07-14.md), **result: MIXED, mechanism NOT supported** (deltas 5–10x the cost saved, non-monotone = timing noise; real effect ≈ cost saved ≤ +13 bps/yr; vmq's internal band already banks it; no adoptable value, no live change; robustness/turnover_band.md) |
| ewma-vol (layer B, EWMA vs realized-window vol estimator inside vol_managed_qqq, matched pair) | 2026-07-14 | 3 (EWMA com ∈ {10 matched-pair decisive, 5, 15.67=λ0.94} through the identical band/cap path) vs the frozen realized-21 baseline | **yes, adaptive loop: targets a blind-promoted book; redteam vol_lookback plateau motivated fixing memory and testing shape** | matched pair only: median paired 12m net delta ≥ +10 bps AND win share ≥ 55% = EWMA better; ≤ −10 bps AND ≤ 45% = realized better; else keep frozen estimator (preregistrations/ewma-vol-2026-07-14.md), **result: REALIZED BETTER** (matched pair −63.6 bps / 35% win share; all 3 memories lose, λ=0.94 worst; the window's hard forget releverages sooner post-spike = feature; frozen spec unchanged; F-024, robustness/ewma_vol.md) |
| jse-factor-gate (layer B, per-factor-gated JSE vs blanket, Estimator Lab; Kristen Stage-0 2026-07-14) | 2026-07-14 | 6 gate configs ((ψ_min, g_min) ∈ {0.3,0.5,0.7}×{0.05,0.10}, k=5) vs jse5/pca5/jse3 baselines; decisive cell = long-only n=63; n=252 + unconstrained reported as diagnostics | **yes, adaptive loop ×2: reacts to F-021 FINAL blanket verdicts and the k=5<k=3 long-only ordering at n=63; gates are the theorem's own conditions (detection threshold, Assumption-3 separation)** | median paired monthly delta (gated − blanket) ≤ −0.2 bps AND p<0.05 AND gated_jse5 recovers ≥ jse3's benefit = helps; within ±0.1 bps = redundant; ≥ +0.1 bps = harmful → FAILURES.md and the stale "JSE k=3-5 unconstrained" queue item retires with it (preregistrations/jse-factor-gate-2026-07-14.md) |
| open-close (layer D, open+close execution engine + overnight-premium exploitability, SPY/QQQ) | 2026-07-14 | 4 strategy books (overnight-only / intraday-only × {SPY, QQQ}) + 2 buy-and-hold controls, gross and net; engine gates: nests harness.run byte-exactly, per-day leg composition identity | **yes, adaptive loop: F-006 supplies the premium estimate and the reopen-by-design promise** | gross overnight share ≥ 60% of buy-hold on both = premium real; net Sharpe (2 bps/side frozen) > buy-hold on both = EXPLOITABLE, else NOT, stock tilt stays closed by cost arithmetic (preregistrations/open-close-2026-07-14.md), **result: PREMIUM REAL (75% share both ETFs), NOT EXPLOITABLE** (loses to buy-hold even gross at equal DD; break-even cost NEGATIVE −0.6/−0.8 bps; F-006 CLOSED FINALLY, F-025; robustness/open_close.md) |
| jse-theorem-complete (layer B, eq.-13-calibrated shrinkage intensity; Kristen Stage-0 + explicit F-026 stop-iterating override, new lab info 2026-07-14) | 2026-07-14 | 2 (rotation-MC distribution ∈ {t6 decisive, normal}, R=500, k=5) vs jse5/pca5; decisive cell long-only n=63; cumulative overlay-line chances 6+2=8 | **yes, adaptive loop: reacts to F-026 (separation binding, median-stat lesson) + F-021 FINAL; override grounded in 2026-07-14 CDAR meeting (eq. 13 complete error model, rotation MC)** | t6 variant median paired delta (theorem-complete − blanket jse5): ≤ −0.3 bps & p<0.05 = calibration helps; ≥ +0.3 bps & p<0.05 = wrong-target evidence; else flat → FAILURES.md (preregistrations/jse-theorem-complete-2026-07-14.md), **result: WRONG TARGET** (worse in every cell, decisive +1.85 bps p=0.0092, unconstrained +30–50 bps; rotation diagnosis validates, q-target response fails with dose-response; multifactor JSE needs per-factor targets = lab arc step 4; F-027, estimator_lab/THEOREM_COMPLETE.md) |
| jse-hedge-pair (layer B measurement + pre-registered revival gate; HYP-005b reopen, Kristen Stage-0, no-revival clause removed at her direction) | 2026-07-14 | 4 new ({pca1, jse1} × n_est ∈ {63, 252}) + sector-ETF baseline rerun; frozen A&L harness (60/1.25/0.5/skip1, PIT, 10 bps/side implementable) | **yes, adaptive loop: reacts to F-027 (factor-1 = JSE's valid target), F-010/F-021 priors, HYP-005 post-mortem engine + cost basis** | decisive jse1 vs pca1 @ n_est=63: median monthly \|β_SPY\| lower with p<0.05 AND gross-Sharpe delta ≥ 0 = hedge improved; revival gate = ANY variant net Sharpe ≥ 0.5 deflated at n_trials=5 → Stage-2 candidacy, Kristen's call (preregistrations/jse-hedge-pair-2026-07-14.md), **result: NO EFFECT / revival NOT HIT** (ψ̂₁≈1: f1 has no correctable bias on large panels, with F-027, the correction is fully bracketed; side finding: PCA-1 hedge doubles gross edge vs sector ETFs, +0.30→+0.58, net −1.02→−0.40 still dead; F-028, estimator_lab/HEDGE_PAIR.md) |
| jse-regime-map (layer B, factor-1 JSE beta-overlay across universes by factor SNR; Kristen Stage-0) | 2026-07-14 | 6 (universe ∈ {large S&P500, mid S&P400, small S&P600} × n_est ∈ {63, 252}); jse1 vs pca1 one-factor min-var, weight-stability metric | **yes, adaptive loop: reacts to F-028 (ψ̂₁≈1 large-cap), F-027 (f1=valid target), F-021 (benefit ~ p/n)** | decisive n_est=63: small-cap jse1 median monthly turnover < pca1 (p<0.05) AND reduction monotone large≥mid≥small = regime-dependent stabilization; mechanism check ψ̂₁ lower on small-cap (preregistrations/jse-regime-map-2026-07-14.md), **result: HARMFUL** (JSE raises turnover on all 3 universes, t=6–12, worst on small-cap; Δvol≈0; ψ̂₁≈0.976–0.996 everywhere ⇒ S&P panels never reach the noisy regime; single-factor JSE's benefit lives nowhere in S&P large/mid/small; F-029, estimator_lab/REGIME_MAP.md) |
| subspace-invariance (layer B diagnostic, is min-var a subspace functional? rotation-invariance test, Avenue 2; Kristen Stage-0) | 2026-07-14 | measurement (3 universes × 2 windows × k∈{1,3,5}); within-subspace rotation CV + proj/flat/rand-subspace gaps, unconstrained min-var | **yes, reacts to F-028/F-029 (dominant market factor = λ-heterogeneity that stresses O(δ²/λ)) + the step-4 subagent synthesis** | decisive large-cap n=63 k=5: within-subspace CV ≤ 0.02 AND rand-subspace control ≥ 0.25 rel-excess AND proj gap ≤ 0.10 = subspace functional confirmed (preregistrations/subspace-invariance-2026-07-14.md), **result: CONFIRMED** (rot CV 1.4%, rand-subspace +26%, proj gap +1.9%; ~19x separation; holds at λ₁/λ₅≈9; min-var is a subspace functional ⇒ step-4 = de-bias subspace+eigenvalues, not the frame; estimator_lab/SUBSPACE_INVARIANCE.md) |
| subspace-averaging (layer B, target-free Grassmannian-mean min-var estimator; Avenue-2 constructive, Kristen Stage-0) | 2026-07-14 | 3 treatment L∈{3,6,12} + L=1 baseline; universe {large/mid/small} × window {63,252}, k∈{1,3,5}; averaged projector P̄ vs single-window P̂, unconstrained min-var realized vol | **yes, reacts to EXP-2026-07-14-subspace-invariance (subspace sufficiency CONFIRMED) + F-028/F-029 (well-pinned market factor ⇒ gains only in weak factors)** | decisive small-cap n=63 k=5: best L median relative realized-vol delta ≤ −0.5% AND p<0.05 AND interior/monotone L-curve = averaging helps; pre-stated limit: averaging cuts variance not the dispersion-bias floor (preregistrations/subspace-averaging-2026-07-14.md), **result: NO EFFECT decisive / net HARMFUL broadly** (averaging worse where subspace unstable, +14.6% large-cap k3 L12 p<0.001; disagreement is DRIFT not noise per the stability diagnostic ⇒ target-free averaging insufficient; step-4 surviving route = bias-aware Avenue 3 rotation-bound SOCP; F-030, estimator_lab/SUBSPACE_AVERAGING.md) |
| robust-rotation-socp (layer B, rotation-bound robust min-var, per-factor trust from Davis-Kahan/t₆; Avenue-3, Kristen Stage-0) | 2026-07-14 | rob_perfactor κ∈{0.5,1,2} + rob_uniform + full + lw baselines; universe {large/mid/small} × window {63,252}, k=5; no-cross-term PSD reduction, reuses F-027 rotation_mc | **yes, reacts to F-030 (drift ⇒ bias-aware down-weighting), subspace-invariance (the tension: min-var ignores within-subspace directions), F-027 (rotation_mc)** | decisive large-cap n=63 k=5 κ=1: (A) rob_perfactor vs full ≤ −0.5% p<0.05 AND (B) vs uniform ≤ −0.25% p<0.05 = per-factor trust helps; else uniform-only/no-effect (preregistrations/robust-rotation-socp-2026-07-14.md), **result: HARMFUL** (rob_perfactor +10.4% vol vs full p=0.003, 5/6 cells worse, monotone in κ; per-factor beats uniform −12% but any rotation distrust costs vol; closes the loop with subspace-invariance, rotation bound has NO positive min-var value; all 3 avenues ruled out, distilled open problem = drift-aware subspace estimator; F-031, estimator_lab/STEP4_SYNTHESIS.md) |
| forward-alpha-prediction (layer D measurement, prediction registered against the alpha-forward experiment's own frozen thresholds) | 2026-07-21 | 0 new variants: 7 per-book point predictions + 2 execution predictions, benchmark menu frozen in the pre-reg | **yes, doubly adaptive: predictions come from a re-reading of the spent holdout, and the books were selected by that same holdout; establishes no alpha, only exposes the reading to being wrong** | count of correct per-book 12m outcomes out of 7 (7/7 supported, 5-6 partial, <=4 the reading is not predictive) + median exec cost in [40,60] bps positive on both sides at n>=20 per side (preregistrations/forward-alpha-prediction-2026-07-21.md), **PENDING**, first review 2027-01-14 |

## Spec-level registry

| # | spec | hunt | status | result summary |
|---|---|---|---|---|
| 1 | vol_managed_qqq | 1 | survivor (stress-passed 5y too) | 1y +42.5%; 5y +23.3% (not blind on 5y) |
| 2 | vol_core_svxy | 1 | survivor (stress) | 1y +36.1%; 5y +24.3% (not blind) |
| 3 | breadth_gated_leverage | 1 | retired | 1y pass on beta; 5y +14.7%, param-fragile |
| 4 | trend_gated_spy_2x | 1 | retired | 5y +16.5%, −40% DD; superseded by trend_vol_qqq |
| 5 | momentum_concentrated | 1 | sleeve-only (WF demoted, F-015) | best beta-matched excess both windows; misses raw bar on 5y |
| 6 | dual_momentum_gem | 1 | retired | 1y star (+58.6%), 5y +17.9%, whipsaw-fragile |
| 7 | svxy_vix_carry | 1 | retired | failed both windows; gap risk as pre-registered |
| 8 | gap_drift | 1 | watch | 1y +5.8% excess; 5y decayed to +12.3% CAGR |
| 9 | ew_levered_vix_gate | 1 | retired | negative excess 1y; 5y +11.5% |
| 10 | deep_dip_reversion | 1 | retired → failure DB | 5y +2.1%; reversion at daily freq confirmed dead |
| 11 | vix_panic_buyer | 1 | retired (WF: −62% GFC window, F-013) | 1y +3.2% excess; 5y +21.2% (not blind) |
| 12 | composite_book | 1 | retired | beta in a costume; −44% DD in 5y |
| 13 | pca_minvar_raw | 1 | control (kept) | Goldberg pair control |
| 14 | pca_minvar_jse | 1 | watch (estimator research) | beat raw by +15bps 1y / +10bps CAGR 5y, direction right, expression muted |
| 15 | tsmom_multi_asset | 2 | survivor (as sleeve only) | 5y blind +10.5% standalone; +13.7% in 2022, crisis alpha |
| 16 | trend_vol_qqq | 2 | survivor (tail-hedge variant, F-014) | 5y blind +24.7%, Sharpe 1.11 |
| 17 | dual_momentum_gold | 2 | survivor (discounted) | 5y blind +29.1%; gold-menu design hindsight |
| 18 | defensive_ensemble | 2 | survivor (capital-preserver: WF median +15%, worst −18%) | 5y blind +19.9%, Sharpe 1.32, −13.4% DD, flat 2022 |
| 19 | EXP reversal × liquidity-shock (H-E1) | IA | measurement, **KILL** (signal-space, L1) | high-demand 21d IC +0.0069 t=0.41 ≈ dead baseline; interaction t<2 both horizons; holdout sign flips → closes NR-1's last named daily-bar angle |
| 20 | EXP MOC vs MOO fill (H-D1) | IA | execution measurement, **KILL** | net B−A paired t=0.17, overnight-gap t=1.25 (both <2), 2022+ holdout sign unstable → keep next-open, close H-D3; no live spec changed |
| 21 | EXP LW constant-corr target (H-lw-target) | IA | estimator, **KILL** | unconstrained lw_cc−identity +87bps t=+3.69 (adverse direction), holdout flips → LW-target docket closed; off-primary long-only −103bps t=−12.5 (stable) noted, not the registered book |
| 22 | EXP residual-diagonal shrink (H-idio-shrink) | IA | estimator, **INCONCLUSIVE** | pca3 unconstr vol −47bps t=−6.32 (holdout stable) BUT non-monotone in α and net Sharpe 0.79→0.69, still loses to MP → vol bought with churn; frozen monotonicity clause unmet |
| 23 | EXP-A bond-carry predictability (Discovery) | DISC | measurement, **REJECTED** (F-022) | carry coef t=1.53<2, rank-IC t=0.43, holdout sign flips; after ΔDGS10 control z-coef→t=1.05 (mechanical duration); β_SPY=−0.144; orthogonality NOT INDEPENDENT (roll_corr 0.737>0.65). No portfolio; free-data carry lane closed |
| 24 | EXP-B conditional-vol mechanism (Discovery) | DISC | mechanism, **UNSUPPORTED** (F-020 addendum) | 3/4 property signs hold & stable (vol-clustering strongest t=3.36) but 4-property model not jointly significant at cluster level (wild-cluster bootstrap p=0.44, G=5); narrows F-020 descriptively (benefit concentrates in high-premium/high-clustering equity+gold), no transportable mechanism |

## Controls / benchmarks (not trials)

| benchmark | purpose |
|---|---|
| SPY buy-and-hold | market base rate (built into every evaluator) |
| bench_qqq_buyhold | naive base for all QQQ specs |
| bench_qqq_sma200_2x | gate-only naive: isolates trend_vol_qqq's gate component |
| vol_managed_qqq (dual role) | vol-target-only naive: isolates trend_vol_qqq's vol component |

## Rules going forward

1. Every new spec gets a row BEFORE its first holdout/walk-forward score.
2. Every adaptive loop (designing after seeing an out-of-sample result) gets a hunt-level
   row with the derivation flagged.
3. Retired specs stay in the table and get a failure-database entry (FAILURES.md).
4. Implementation claims are reported as the delta vs the naive benchmark, not the raw return.

> 2026-07-10 (later): statuses revised by the 82-window walk-forward (walkforward/summary.md); see FAILURES.md F-013..F-015.

## 2026-07-14 program additions (autonomous JS/factor-risk program, Kristen Stage-0 via program directive)

| Exp | Date | Variants registered | Derived from prior holdout results? | Decisive stat |
|---|---|---|---|---|
| factor-attribution (layer B diagnostic, live books vs known factors) | 2026-07-14 | 1 experiment: 4 nested models × 7 books + 4 controls, all reported | **yes, adaptive loop: targets blind-promoted books** | blind-window M2 NW-t ≥ 2 per book, Bonferroni m=3 tier at t ≥ 2.4 (preregistrations/factor-attribution-2026-07-14.md), **result: NO CANDIDATE** (best defensive_ensemble t=1.77; adversarial review decomposes all 4 best cells to ≤2%/yr or t<1.2 residual; verdict "factor-premium harvesting with some unexplained residual return"; research/attribution/) |
| jse-benchrel (layer B, benchmark-relative TE adjudication, Estimator Lab; cumulative JSE-line chances 9) | 2026-07-14 | 1 one-shot run, thresholds from the adversarial review | **yes, adaptive loop: adjudicates the one UNTESTED boundary-map cell after review of completed results** | jse5−pca5 paired TE: overturn iff rel ≤ −0.5%, p<0.05, abs ≤ −10 bps (preregistrations/jse-benchrel-2026-07-14.md), **result: VERDICT STANDS** (ΔTE −0.04 bps, p=0.42; MP best again; estimator_lab/BENCHREL.md) |

## Data correction (2026-07-14, not a trial)

Phantom 2026-05-25 (^VIX-only) row in holdout.parquet / holdout5y.parquet corrupted all
frozen book returns after that date (~6 weeks). Builder fixed, row dropped surgically,
originals archived, every frozen number recomputed: **no pass/fail set changes, no
promoted-book changes; two beta-matched-excess sign flips on non-promoted specs
(deep_dip_reversion 1y −4.2%→+8.1%, vol_core_svxy −1.1%→+1.2%); corrected SPY 1y +21.97%.**
Correction of record: memos/panel-phantom-row-correction.md. Frozen results/*.json remain
as-run (write-once); cite corrected values from the memo for the affected stretch.

| alpha-forward (layer D measurement, forward alpha-isolation vs frozen M2/M2g replication; implements the 2026-07-14 program's production action) | 2026-07-14 | 1 forward experiment: 7 books + 4 placebos, thresholds frozen pre-start | **yes, adaptive loop: implements the factor-attribution program's recommendation** | 12m NW t ≥ 2.4 (m=3 clusters) + both-halves positive + placebo bands + financing stress = promising-but-unproven → independent replication; t < 2.0 or negative = factor-harvesting confirmed → factor-adjusted kill rules (memos/alpha-forward-setup-2026-07-14.md), **ACCUMULATING** from 2026-07-15, first finalized days on FF publication |
